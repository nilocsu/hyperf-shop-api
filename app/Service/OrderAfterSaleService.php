...
//     * @return array
//     */
//    public  function orderGoodsRow($order_id, $order_detail_id, $user_id)
//    {
//        // 获取列表
//        $data_params = array(
//            'm'         => 0,
...
//            ],
//        );
//        $ret =  $this->orderService->orderList($data_params);
...
//     * @return array
//     */
//    public  function aftersaleCreate($params = [])
//    {
//        // 请求参数
//        $p = [
//            [
...
//
//        // 获取订单数据
//        $order = $this->ordferGoodsRow($params['order_id'], $params['order_detail_id'], $params['user']['id']);
//        if($order['code'] != 0)
//        {
//            return $order;
//        }
...
//            ['status', '<=', 2],
//        ];
//        $count = (int) Db::table('order_aftersale')->where($where)->count();
//        if($count > 0)
//        {
...
//
//        // 历史退款金额
//        $history_price = PriceNumberFormat(Db::table('order_aftersale')->where($where)->sum('price'));
...
//        $history_number = (int) Db::table('order_aftersale')->where($where)->sum('number');
//        if($params['type'] == 1)
//        {
...
//            'apply_time'        => time(),
//        ];
//        if(Db::table('order_aftersale')->insertGetId($data) > 0)
//        {
...
//     * @return array
//     */
//    public  function aftersaleDelivery($params = [])
//    {
//        // 请求参数
//        $p = [
//            [
...
//            'user_id'   => $params['user']['id'],
//        ];
//        $aftersale = Db::table('order_aftersale')->where($where)->first();
//        if(empty($aftersale))
//        {
...
//            'upd_time'          => time(),
//        ];
//        if(Db::table('order_aftersale')->where($where)->update($data))
//        {
...
//     * @return array
//     */
//    public  function OrderAftersaleList($params = [])
//    {
...
//
//        // 获取数据列表
//        $data = Db::table('order_aftersale')->select($field)->where($where)->limit($n)->offset($m*$n)->orderBy($order_by)->select();
//        if(!empty($data))
//        {
...
//            {
//                // 订单商品
//                $order = $this->ordferGoodsRow($v['order_id'], $v['order_detail_id'], $v['user_id']);
...
//     * @return array
//     */
//    public  function OrderAftersaleListWhere($params = [])
//    {
//        // 用户类型
...
//            if(empty($params['user']))
//            {
//                $user_ids = Db::table('User')->where('username|nickname|mobile|email', '=', $params['keywords'])->column('id');
//                if(!empty($user_ids))
//                {
...
//     */
//    public  function OrderAftersaleTotal($where = [])
//    {
//        return (int) Db::table('order_aftersale')->where($where)->count();
//    }
//
//    /**
//     * 订单售后取消
...
//
//        // 售后订单
//        $aftersale = Db::table('order_aftersale')->where($where)->find();
//        if(empty($aftersale))
//        {
//            return dataReturn('数据不存在或已删除', -1);
//        }
...
//
//        // 数据更新
//        if(Db::table('order_aftersale')->where($where)->update(['status'=>5, 'cancel_time'=>time(), 'upd_time'=>time()]))
//        {
...
//     * @param   [array]          $params [输入参数]
//     */
//    public  function AftersaleConfirm($params = [])
//    {
//        // 请求参数
//        $p = [
//            [
...
//
//        // 数据更新
//        if(Db::table('order_aftersale')->where($where)->update(['status'=>1, 'confirm_time'=>time(), 'upd_time'=>time()]))
//        {
...
//     * @param   [array]          $params [输入参数]
//     */
//    public  function AftersaleAudit($params = [])
//    {
//        // 请求参数
//        $p = [
//            [
...
//
//        // 售后订单
//        $aftersale = Db::table('order_aftersale')->where(['id' => intval($params['id'])])->find();
//        if(empty($aftersale))
//        {
//            return dataReturn('数据不存在或已删除', -1);
//        }
...
//
//        // 订单支付方式校验
//        $pay_log = Db::table('PayLog')->where(['order_id'=>$order['data']['id'], 'business_type'=>1])->find();
//
//        // 手动处理不校验支付日志
...
//        if($is_walet === true)
//        {
//            $wallet = Db::table('Plugins')->where(['plugins'=>'wallet'])->find();
//            if(empty($wallet))
//            {
...
//        }
//        if(!Db::table('Order')->where(['id'=>$order['data']['id']])->update($order_upd_data))
//        {
//            Db::rollback();
...
//            'upd_time'          => time(),
//        ];
//        if(!Db::table('OrderDetail')->where(['id'=>$aftersale['order_detail_id']])->update($detail_upd_data))
//        {
//            Db::rollback();
...
//             $this->orderService->OrderHistoryAdd($order['data']['id'], $order_upd_data['status'], $order['data']['status'], '关闭', $creator, $creator_name);
//        }
//
//        // 更新退款状态
//        $aftersale_upd_data = [
...
//            'upd_time'      => time(),
//        ];
//        if(!Db::table('order_aftersale')->where(['id'=>$aftersale['id']])->update($aftersale_upd_data))
//        {
//            Db::rollback();
...
//     */
//    private  function OriginalRoadRefundment($params, $aftersale, $order, $pay_log)
//    {
//        // 支付方式
...
//     * @param   [array]          $pay_log   [订单支付日志]
//     */
//    private  function WalletRefundment($params, $aftersale, $order, $pay_log)
//    {
//        // 获取用户钱包校验
...
//            'upd_time'          => time(),
//        ];
//        if(!Db::table('PluginsWallet')->where(['id'=>$user_wallet['data']['id']])->update($data))
//        {
...
//     * @param   [array]          $params [输入参数]
//     */
//    public  function AftersaleRefuse($params = [])
//    {
//        // 请求参数
//        $p = [
//            [
...
//            'upd_time'          => time(),
//        ];
//        if(Db::table('order_aftersale')->where(['id' => intval($params['id'])])->update($data))
//        {
...
//     * @param   [array]          $params [输入参数]
//     */
//    public  function AftersaleDelete($params = [])
//    {
//        // 请求参数
//        $p = [
//            [
...
//
//        // 删除操作
//        if(Db::table('order_aftersale')->where(['id' => intval($params['id'])])->delete())
//        {
...
//     */
//    public  function OrderAftersaleCalculation($order_id, $order_detail_id)
//    {
//        // 可退数量/金额
//        $returned_quantity = 0;
//        $refund_price = 0.00;
...
//
//        // 获取订单数据
//        $order = Db::table('Order')->where(['id'=>$order_id])->field('id,status,pay_status,buy_number_count,increase_price,preferential_price,price,total_price,pay_price,refund_price,returned_quantity')->find();
//        if(!empty($order))
//        {
//            $dateil = Db::table('OrderDetail')->where(['order_id'=>$order_id])->field('id,price,total_price,buy_number,refund_price,returned_quantity')->select();
//            if(!empty($dateil))
//            {
//                foreach($dateil as $v)
//                {
...
//                        ['status', '<=', 2],
//                    ];
//                    $aftersale = Db::table('order_aftersale')->where($where)->field('sum(number) as number, sum(price) as price')->find();
